...
Run training on TPU:
```bash
accelerate launch --config_file scripts/accelerate_configs/tpu.yaml \
    examples/llada/sft.py --model_name_or_path GSAI-ML/LLaDA-8B-Base
```

> **Note:** Flash attention and 4-bit quantization (`bitsandbytes`) are not available on TPU. The code automatically falls back to PyTorch's SDPA for attention.
